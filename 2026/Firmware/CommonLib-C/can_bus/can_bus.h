...
static inline void Can_Init(CanBus *self, CAN_HandleTypeDef *hcan,
                            uint32_t myId) {
  self->hcan = hcan;
  self->myId = myId;
  ...
  if (HAL_CAN_ConfigFilter(self->hcan, &filter) != HAL_OK)
    Error_Handler(); // フィルター設定
  if (HAL_CAN_ActivateNotification(self->hcan, CAN_IT_RX_FIFO0_MSG_PENDING) !=
      HAL_OK)
    Error_Handler(); // 受信割り込み設定
}
...
static inline void Can_Send(CanBus *self, const CanData *canData) {
  if (HAL_CAN_GetTxMailboxesFreeLevel(self->hcan) > 0) {
    ...
    HAL_CAN_AddTxMessage(self->hcan, &self->txHeader, (uint8_t *)canData->data,
                         &TxMailbox);
  }
}

static inline void Can_Recv(CanBus *self, CanData *canData) {
  if (HAL_CAN_GetRxMessage(self->hcan, CAN_RX_FIFO0, &self->rxHeader,
                           canData->data) == HAL_OK) {
    canData->stdId = (self->rxHeader.IDE == CAN_ID_STD) ? self->rxHeader.StdId
                                                        : self->rxHeader.ExtId;
  }
}
