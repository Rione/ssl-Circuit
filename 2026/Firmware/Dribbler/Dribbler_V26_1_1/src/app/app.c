...
void MainApp() {
  ...
  while (1) {
    ...
    Motor_UpdateTorqueControl(adc_val[MOTOR_CURRENT_IDX]);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef* hcan) {
  if (can.hcan == hcan) {
    ...
    if (canRecvData.stdId == 0x20) {
      Motor_Drive(canRecvData.data[0] / 255.0f);
    }
  }
}
...
void CAN_Data_Output_ID0x1d2_246() {
  ...
  uint8_t current_change_detected = 0;
  ...
  current_val /= 100;
  current_change_detected = Motor_IsBallCapturedByCurrentChange((uint16_t)current_val, (Change_Motor_Speed != 0));
  ...
  if (!Halt_CAN_Data_Send) {
    ...
    if (Change_Motor_Speed == 0) {
      if (photo == 1) {
        if (current_change_detected == 1 || (current_change_detected == 0 && get_ball == 1)) {
          get_ball = 1;
