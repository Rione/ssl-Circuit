...
OmniDrive::OmniDrive(BufferedSerial* serials) {
  for (int i = 0; i < 4; i++)
    serials_[i] = &serials[i];
}

void OmniDrive::SetVel(int16_t vel_x, int16_t vel_y, int16_t vel_angle) {
...
void OmniDrive::Recv() {
  const uint8_t kHeader = 0xFF;
  const uint8_t kFooter = 0xAA;
  const uint8_t kDataSize = 3;  // 受信データサイズ(バイト数)
  static uint8_t recv_data[4][kDataSize];
  ...
  for (int i = 0; i < 4; i++) {
    if (serials_[i]->available()) {
      ...
      if (index[i] == 0) {
        if (recv_byte[i] == kHeader) {  // ヘッダー確認
          index[i]++;
        } else {
          index[i] = 0;
        }
      } else if (index[i] == (kDataSize + 1)) {
        if (recv_byte[i] == kFooter) {  // フッター確認
          // 正常にデータを受信
          emg_ = recv_data[i][0] & 0b00000001;           // 非常停止信号
          ready_ = (recv_data[i][0] >> 1) & 0b00000001;  // ドライバ準備完了信号
          vel_wheel_angular_[i] = (int16_t)((recv_data[i][1] << 8) |
                                            recv_data[i][2]);  // 角速度[rad/s]
        }
        index[i] = 0;
      } else {
...
void OmniDrive::GetVel(int16_t* vel_x, int16_t* vel_y, int16_t* vel_angle) {
  ...
  for (int i = 0; i < 4; i++) {
    vx_sum +=
        v_wheel_linear[i] * (-MyMath::sinDeg(robot_params::kMotorDegree[i]));
