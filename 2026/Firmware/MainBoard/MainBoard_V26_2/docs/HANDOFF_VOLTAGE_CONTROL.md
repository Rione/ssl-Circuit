...
- `main_mode.c` は今このテストを呼んでいる（起動5秒後に車輪が回るので、浮かせておくこと）。
...
5. **MainBoardで電圧制御を実装する**。段階的に:
   - (a) フィードフォワードのみ: 電圧 = 逆起電力定数×ω ＋ 加速に必要な電圧 ＋ 摩擦補償 → **実装し、浮かせて確認済み（5.5）。次は床でTCSテストを電圧制御で走らせ、4.2と比べる。加速ぶん（`WHEEL_VOLT_KA`）は、そのデータの加速区間から決める。**
