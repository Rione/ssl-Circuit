...
// traction_limit_v) を約0.2秒で 1.5V→3.6V までなめらかに上げて加速し、IMU の加速度が一番大きくなった
// ときのトルク (ピーク) と、車輪と IMU の加速度が離れ始めたときのトルク (滑り始め) を向きごとに測る。
// 続けてブレーキも同じように測る。
//  - 向き: 前・後・左・右・斜め4方向・その場旋回の左右 (10本)。2本ずつ組にして、1本目は原点から
//    空きのある方へ、2本目は戻る向きへ走る。組ごとに低速で原点に戻る
//  - これを2回くり返し、ピークの差が大きい向きの組だけ3回目を測る。3回目でも差が大きい向きは
//    「不安定」と記録する
//  - トルクは止まった状態から短時間で上げる。速度が出ると、今の回転数で転がり続ける電圧が増えて
//    電圧上限 (4.9V) までの余裕が減り、トルクを上げられなくなるため (前進で約0.8m/s を超えると 3.6V を
//    かけられない)
...
typedef enum {
  RAMP_END_NONE = 0,
  RAMP_END_ONSET = 1,     // 滑り始めを見つけた
  RAMP_END_MAX_V = 2,     // 上限 (3.6V) まで上げても滑らなかった
