...
void Robot::uiSendSerial(RobotInfo_t &info, uint16_t interval) {
      ...
      buffer[3] = info.dribbleStatus.isDetectedBall ? 1 : 0;
      
      // Convert int16_t to float and copy to buffer
      float m0 = (float)info.mdStatus.motorAngularVelocity[0];
      float m1 = (float)info.mdStatus.motorAngularVelocity[1];
      float m2 = (float)info.mdStatus.motorAngularVelocity[2];
      float m3 = (float)info.mdStatus.motorAngularVelocity[3];
      
      memcpy(&buffer[4], &m0, 4);
      memcpy(&buffer[8], &m1, 4);
      memcpy(&buffer[12], &m2, 4);
      memcpy(&buffer[16], &m3, 4);
