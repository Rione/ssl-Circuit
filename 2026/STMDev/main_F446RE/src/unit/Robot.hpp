...
class Robot {
     public:
      ...
      Average<float> meanVelX = Average<float>(info.meanVelXBuf, 15);
      Average<float> meanVelY = Average<float>(info.meanVelYBuf, 15);
      Average<float> meanVelAngler = Average<float>(info.meanVelAngBuf, 15);
     ...
     private:
      ...
      Median<uint16_t> medianPhotoValue = Median<uint16_t>(photoSensorValueBuff, 15);

      uint8_t batteryValueBuff[15];
      Median<uint8_t> medianBatteryValue = Median<uint8_t>(batteryValueBuff, 15);
};
