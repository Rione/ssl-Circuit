...
int8_t BMI270::begin()
{
    // Variable to track errors returned by API callsa
    int8_t err = BMI2_OK;
...
*/
int8_t BMI270::beginI2C(uint8_t address, I2C_HandleTypeDef* hi2c)
{
    interfaceData.interface  = BMI2_I2C_INTF;
    interfaceData.i2cAddress = address;
    interfaceData.hi2c       = hi2c;
...
/// @return Error code (0 is success, negative is failure, positive is warning)
BMI2_INTF_RETURN_TYPE BMI270::readRegistersI2C(uint8_t reg,uint8_t* data,uint32_t len,BMI270_InterfaceData* ifc)
{
    /*
    ...
    */
    HAL_StatusTypeDef ret =
        HAL_I2C_Mem_Read(
            ifc->hi2c,
            ifc->i2cAddress << 1,          // HAL は 8bit address
            reg,
    ...
    return (ret == HAL_OK) ? BMI2_OK : BMI2_E_COM_FAIL;
}
...
BMI2_INTF_RETURN_TYPE BMI270::readRegistersSPI(uint8_t regAddress, uint8_t* dataBuffer, uint32_t numBytes, BMI270_InterfaceData* interfaceData)
{
    ...
    HAL_GPIO_WritePin(GPIOA, interfaceData->spiCSPin, GPIO_PIN_RESET);
    ...
    HAL_GPIO_WritePin(GPIOA, interfaceData->spiCSPin, GPIO_PIN_SET);    
...
/// @param interfaceData Pointer to interface data, see BMI270_InterfaceData
/// @return Error code (0 is success, negative is failure, positive is warning)
BMI2_INTF_RETURN_TYPE BMI270::writeRegistersI2C(uint8_t reg,const uint8_t* data,uint32_t len,BMI270_InterfaceData* ifc)
{
    /*
    // Begin transmission
    interfaceData->i2cPort->beginTransmission(interfaceData->i2cAddress);
...
void BMI270::usDelay(uint32_t period, void* interfacePtr)
{
    ...
    uint32_t cycles = period * (SystemCoreClock / 1000000);
