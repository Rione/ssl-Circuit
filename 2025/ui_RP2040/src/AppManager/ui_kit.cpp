...
void UiKit::infoTab(bool forceUpdate) {
  ...
  if (millis() - timeUpdate > 1000 &&
          info.batteryVoltage != infoPrev.batteryVoltage ||
      forceUpdate) {
    timeUpdate = millis();
  ...
  if (changeFlag_overMode || forceUpdate) {
    ...
    // display.sprite->drawString("Ri-one", 36, 18); //
    // 逕ｻ蜒上′縺ｪ縺・◆繧√ユ繧ｭ繧ｹ繝医〒莉｣逕ｨ 譁ｰ縺励＞繝ｭ繧ｴ繧・70x22
    // 縺ｫ邵ｮ蟆上＠縺ｦ謠冗判 (閭梧勹縺ｮ逋ｽ 0xFFFF
    // 繧帝城℃)
    int logoW = 70;
    int logoH = 22;
    int offsetX = 1;
    ...
    uint16_t eStopBg =
        display.sprite->color565(248, 215, 218); // Light red background
